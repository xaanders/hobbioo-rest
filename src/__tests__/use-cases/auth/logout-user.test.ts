...
describe("Logout User Use Case", () => {
  ...
  it("should throw AuthError when session ID is empty", async () => {
    ...
    await expect(logoutUserUseCase("")).rejects.toThrow(AuthError);
    await expect(logoutUserUseCase("")).rejects.toThrow("Session ID is required");
    expect(mockCognitoAuth.logoutUser).not.toHaveBeenCalled();
  });

  it("should throw AuthError when logout fails", async () => {
    mockCognitoAuth.logoutUser.mockRejectedValue(new AuthError("Failed to logout"));
    const logoutUserUseCase = logoutUser(mockCognitoAuth);

    await expect(logoutUserUseCase("valid-session-id")).rejects.toThrow(AuthError);
    await expect(logoutUserUseCase("valid-session-id")).rejects.toThrow("Failed to logout");
  });
});
