...
export const registerUserController =
  (createUser: CreateUserFn, cognitoAuth: ICognitoAuth) => async (httpRequest: HttpRequest): Promise<HttpResponse> => {
    const { first_name, last_name, email, user_type, password } = httpRequest.body as { first_name: string, last_name: string, email: string, user_type: 1 | 2, password: string };
    ...
    try {
      ...
      if (!cognitoUser) throw new AuthError("Couldn't register user. Please one more time.", JSON.stringify({ user, cognitoUser }));
