...
import { CognitoIdentityProvider, InitiateAuthCommandInput, SignUpCommandInput, SignUpCommandOutput } from '@aws-sdk/client-cognito-identity-provider';
...
export interface ICognitoAuth {
  ...
  registerUser(user: {username: string, password: string, id: string, name: string}): Promise<SignUpCommandOutput>;
...
export function createCognitoAuth(
  ...
  async function getPublicKey(token: string): Promise<string> {
    ...
    if (!decodedToken || typeof decodedToken === 'string' || !decodedToken.header.kid) 
      throw new AuthError('Invalid token', 'Decoded token is invalid: ' + JSON.stringify(decodedToken));
  ...
  async function authenticateUser(username: string, password: string): Promise<string> {
    ...
    if (!expiresIn) throw new AuthError('Authentication failed', 'No response.AuthenticationResult?.ExpiresIn found');
  ...
  async function registerUser(user: {username: string, password: string, id: string, name: string}): Promise<SignUpCommandOutput> {
